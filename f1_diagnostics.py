# f1_diagnostics.py (split-aware, robust)
...
if PARENT_MODE:
    ...
    entries["perf"] = 1.0 - (entries["time_gap_s"] - g.transform("min")) / (
        g.transform("max") - g.transform("min") + eps
    )
    ...
    # Trace plots (only for vars that exist)
    all_vars = list(idata.posterior.data_vars)
    core_vars = ["alpha", "beta", "sigma", "sigma_driver", "sigma_team", "sigma_race_i"]
    var_names = [v for v in core_vars if v in all_vars]
    ...
    plt.errorbar(
        ...
        y=ypos,
        xerr=[
            top_drv["mean"] - top_drv["hdi_3%"],
            top_drv["hdi_97%"] - top_drv["mean"],
        ],
        fmt="o",
        capsize=3,
    )
    ...
    plt.errorbar(
        ...
        y=ypos,
        xerr=[
            top_tm["mean"] - top_tm["hdi_3%"],
            top_tm["hdi_97%"] - top_tm["mean"],
        ],
        fmt="o",
        capsize=3,
    )
    ...
    drivers_map = pd.read_csv(os.path.join(DATA_DIR, "drivers.csv"))[
        ["driver_id", "driver_name"]
    ]
    constructors_map = pd.read_csv(os.path.join(DATA_DIR, "constructors.csv"))[
        ["constructor_id", "constructor_name"]
    ]

    drv_mean = (
        drv[["driver_id", "mean"]]
        .rename(columns={"mean": "driver_eff"})
        .merge(drivers_map, on="driver_id", how="left")
    )
    tm_mean = (
        tm[["constructor_id", "mean"]]
        .rename(columns={"mean": "team_eff"})
        .merge(constructors_map, on="constructor_id", how="left")
    )

    merged = (
        entries.merge(
            drv_mean[["driver_id", "driver_eff", "driver_name"]],
            on="driver_id",
            how="left",
        )
        .merge(
            tm_mean[["constructor_id", "team_eff", "constructor_name"]],
            on="constructor_id",
            how="left",
        )
    )

    by_driver = merged.groupby(
        ["driver_id", "driver_name"], as_index=False
    )[["driver_eff", "team_eff", "perf"]].mean()
...
if DYNAMIC_MODE:
    ...
    if "perf" not in r.columns and {"time_gap_s", "race_id"}.issubset(set(r.columns)):
        ...
        r["perf"] = 1.0 - (r["time_gap_s"] - g.transform("min")) / (
            g.transform("max") - g.transform("min") + eps
        )
    ...
    if {"driver_eff_mean", "team_eff_mean"}.issubset(set(r.columns)):
        drv_scatter = (
            r.groupby(["driver_id", "driver_name"], as_index=False)
            .agg(
                driver_eff=("driver_eff_mean", "mean"),
                team_eff=("team_eff_mean", "mean"),
            )
        )
        plt.figure(figsize=(7, 7))
        plt.scatter(
            drv_scatter["team_eff"], drv_scatter["driver_eff"], s=30, alpha=0.8
        )
        for _, row in (
            drv_scatter.sort_values("driver_eff", ascending=False).head(12).iterrows()
        ):
            plt.text(
                row["team_eff"],
                row["driver_eff"],
        ...
        savefig(
            os.path.join(
                FIG_DIR, f"driver_vs_constructor_dynamic_{SPLIT_TARGET}.png"
            )
        )
        ...
        savefig(
            os.path.join(FIG_DIR, f"drivers_outperforming_dynamic_{SPLIT_TARGET}.png")
        )

    # Calibration scatter: predicted rank vs actual finish (if finish_position present)
    if "finish_position" in r.columns:
        cal = r[
            ["race_id", "driver_id", "pred_perf", "finish_position"]
        ].copy()
        cal["pred_rank"] = cal.groupby("race_id")["pred_perf"].rank(
        ...
        savefig(
            os.path.join(
                FIG_DIR, f"calibration_predrank_vs_finish_{SPLIT_TARGET}.png"
            )
        )

    # If probabilities exist, plot reliability (mini) — only if we can join actual finishes
    ...
    if os.path.exists(probs_path):
        ...
        if (
            {"race_id", "driver_id", "p_win"}.issubset(p.columns)
            and "finish_position" in r.columns
        ):
            # bring in finish_position from r
            p = p.merge(
                r[["race_id", "driver_id", "finish_position"]],
                on=["race_id", "driver_id"],
                how="left",
            )

            if "finish_position" in p.columns:
                p["win"] = (
                    p.groupby("race_id")["finish_position"].transform("min")
                    == p["finish_position"]
                ).astype(int)

                # decile reliability
                q = pd.qcut(
                    p["p_win"], q=10, labels=False, duplicates="drop"
                )
                grp = (
                    p.groupby(q, observed=True)
                    .agg(
                        p_mean=("p_win", "mean"),
                        y_rate=("win", "mean"),
                    )
                ...
                plt.xlabel("Predicted win prob (bin mean)")
                plt.ylabel("Empirical win rate")
                plt.title(f"{SPLIT_TARGET}: Reliability — Win Prob")
                savefig(
                    os.path.join(FIG_DIR, f"reliability_win_{SPLIT_TARGET}.png")
                )
        else:
            print(
                "[WARN] Skipping probability reliability: missing "
                "p_win or finish_position."
            )
