# f1_parent_model.py (split-aware, robust to schema variants)
...
# ---- Normalize races schema: season_id, round, date ----
# season_id
if "season_id" not in races.columns:
    if "season" in races.columns:
        races = races.rename(columns={"season": "season_id"})
    elif "year" in races.columns:
        races = races.rename(columns={"year": "season_id"})
    else:
        # Fallback: try to infer from race_id like "2018_AUS_..."
        if "race_id" in races.columns:
            races["season_id"] = races["race_id"].astype(str).str.slice(0, 4).astype(int)
        else:
            races["season_id"] = 0

# round
if "round" not in races.columns:
    if "race_round" in races.columns:
        races = races.rename(columns={"race_round": "round"})
    else:
        # Fallback: assign a per-season counter by race_id order
        races = races.sort_values(["season_id", "race_id"]).copy()
        races["round"] = races.groupby("season_id").cumcount() + 1

# date
if "date" not in races.columns:
    # fabricate a date based on season and round (every 14 days)
    races = races.copy()
    races["date"] = (
        pd.to_datetime(races["season_id"].astype(str) + "-01-01")
...
# merge core fields
race_merge_cols = ["race_id", "season_id", "round", "track_id", "weather", "date"]
race_merge_cols = [c for c in race_merge_cols if c in races.columns]
...
df = (
    ...
    .merge(races[race_merge_cols], on="race_id", how="left")
)

# keep only classified rows with time gaps
df = df[(df.get("classified", True) == True) & (~df["time_gap_s"].isna())].copy()

# perf target (higher=better)
eps = 1e-6
g = df.groupby("race_id")["time_gap_s"]
df["perf"] = 1.0 - (df["time_gap_s"] - g.transform("min")) / (
...
# ensure season_id / round / date exist on df for sorting
if "season_id" not in df.columns:
    # backfill from races if needed
    df = df.merge(
        races[["race_id", "season_id"]].drop_duplicates(),
        on="race_id",
        how="left",
        suffixes=("", "_races")
    )
    if "season_id_races" in df.columns:
        df["season_id"] = df["season_id"].fillna(df["season_id_races"])
        df = df.drop(columns=["season_id_races"])

if "round" not in df.columns:
    df = df.merge(
        races[["race_id", "round"]].drop_duplicates(),
        on="race_id",
        how="left",
        suffixes=("", "_races")
    )
    if "round_races" in df.columns:
        df["round"] = df["round"].fillna(df["round_races"])
        df = df.drop(columns=["round_races"])

if "date" not in df.columns:
    df = df.merge(
        races[["race_id", "date"]].drop_duplicates(),
        on="race_id",
        how="left",
        suffixes=("", "_races")
    )
    if "date_races" in df.columns:
        df["date"] = df["date"].fillna(df["date_races"])
        df = df.drop(columns=["date_races"])
...
RUN_MODE = (
    ...
    else ("fit" if SPLIT_TARGET == "TRAIN" else "predict")
)
...
def write_posterior_tables(idata, drv_levels, tm_levels, race_levels, feat_names):
    ...
    pd.DataFrame({"race_id": race_levels}).to_csv(
        os.path.join(OUT_DIR, "posterior_races.csv"), index=False
    )
    ...
    pd.DataFrame(rows).to_csv(
        os.path.join(OUT_DIR, "posterior_coefs.csv"), index=False
    )
